...
def Object_tracking(YoloV3, video_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors='', Track_only = []):
    ...
    count_frames = int(vid.get(cv2.CAP_PROP_FRAME_COUNT))
    ...
    while True : # while True
        ...
        #image = cv2.putText(image, "Time: {:.1f} FPS".format(fps_detect), (0, 30), cv2.FONT_HERSHEY_COMPLEX_SMALL,
        # 1, (0, 0, 255), 2)
        ids = []
        ...
        if i % fps == 0 :
            ...
            for box in tracked_bboxes :
                ids.append(str(box[4]))
            with open('night_video.csv', 'a', newline='') as file :
                data_writer = csv.writer(file,delimiter=',')
                data_writer.writerow(ids)
...
Object_tracking(yolo, '/home/vladislav/PycharmProjects/YoloModel/TensorFlow-2.x-YOLOv3/22sec_my.mp4',
                "del.mp4",
                input_size=416, show=True, iou_threshold=0.2,
                rectangle_colors=(255,0,0), Track_only = ["car","truck","bus"])
#["car","truck","bus"]